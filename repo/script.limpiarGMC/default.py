...
def clearCache():
        ...
        if os.path.exists(clear_cache_path)==True:    
            for root, dirs, files in os.walk(clear_cache_path):
                ...
                if file_count > 0:
                    ...
                    dialog = xbmcgui.Dialog()
                    if dialog.yesno("Gargantúa",str(file_count) + "%s Archivos Cache Encontrados[CR]Desea Eliminarlos?" %(entry.name)):
                        for f in files:
                            os.unlink(os.path.join(root, f))
                        for d in dirs:
                            shutil.rmtree(os.path.join(root, d))
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("Gargantúa", "Todos los Archivos se Limpiaron con Exito")
...
def deleteThumbnails():
    ...
    if os.path.exists(thumbnailPath)==True:
            ...
            if choice1 == 1:
                ...
                choice2 =dialog.yesno("Gargantúa", "Desea reiniciar Gargantúa ahora para terminar el proceso")
                if choice2 == 1:
                ...
                if dialog.yesno("Borrar Archivos en Temp", str(file_count) + " Archivos Encontrados[CR]Desea Eliminarlos?"):
                    ...
                    for d in dirs:
                        try:
                            ...
                            pass
        xbmcgui.Dialog().notification('Gargantúa', "Archivos Temp Eliminados")
        ...
        for root, dirs, files in os.walk(purgePath):
            ...
            if file_count > 0:            
                ...
                for d in dirs:
                    shutil.rmtree(os.path.join(root, d))
                dialog = xbmcgui.Dialog()
                dialog.ok("Gargantúa", "Borrar todo el contenido de Paquetes")
            else:
                dialog = xbmcgui.Dialog()
                dialog.ok("Gargantúa", "Eliminados Paquetes")
    ...
    if os.path.exists(indigoPath)==True:    
        for root, dirs, files in os.walk(indigoPath):
            ...
            if file_count > 0:
                ...
                xbmcgui.Dialog().notification('Gargantúa', "Indigo [COLOR green] Desinstalado[/COLOR]")
                pass
    ...
    if os.path.exists(gdrivePath)==True:    
        for root, dirs, files in os.walk(gdrivePath):
            ...
            if file_count > 0:
                ...
                xbmcgui.Dialog().notification('Gargantúa', "gdrive [COLOR green] Desinstalado[/COLOR]")
                pass
...
def enableadaptive():
        ...
        xbmc.executebuiltin('XBMC.Notification(%s, %s, %s, %s)' % ('Gargantúa' , 'Input Stream Adaptative[COLOR green] Instalado[/COLOR]' , '3000', icon))
...
def enablertmp():
        ...
        xbmc.executebuiltin('XBMC.Notification(%s, %s, %s, %s)' % ('Gargantúa' , 'InputStream RTMP[COLOR green] Instalado[/COLOR]' , '3000', icon))
...
def compactDB():
    ...
    xbmcgui.Dialog().ok('Gargantúa', msg)
    return
