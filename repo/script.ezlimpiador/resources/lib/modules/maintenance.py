        ...
        if os.path.exists(clear_cache_path)==True:
            for root, dirs, files in os.walk(clear_cache_path):
                ...
                if file_count > 0:
                    ...
                    pass
    mensaje = str(contador) + ' Ficheros de Cache borrados'
    if mode == 'verbose': xbmc.executebuiltin('Notification(%s, %s, %s, %s)' % ('EZ-Limpiador' , mensaje , '3000', iconpath))
...
def deleteThumbnails(mode='verbose'):
    ...
    try:
        ...
        pass
    mensaje = str(contador) + ' Imagenes Eliminadas'
    if mode == 'verbose': xbmc.executebuiltin('Notification(%s, %s, %s, %s)' % ('EZ-Limpiador' , mensaje , '3000', iconpath))
...
def purgePackages(mode='verbose'):
    ...
    for root, dirs, files in os.walk(purgePath):
        ...
        if file_count > 0:
            ...
            contador += file_count
    mensaje = str(contador) + ' Paquetes Borrados'
    if mode == 'verbose': xbmc.executebuiltin('Notification(%s, %s, %s, %s)' % ('EZ-Limpiador' , mensaje , '3000', iconpath))
