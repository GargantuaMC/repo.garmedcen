...
def subMenu_maintenance():
	addDir('Limpiar Packages','',6,addon_icon,addon_fanart,local_string(30005),isFolder=False)
	addDir('Limpiar Caratulas','',7,addon_icon,addon_fanart,local_string(30008),isFolder=False)
...
def main(NAME, NAME2, VERSION, URL, ICON, FANART, DESCRIPTION):
	...
	yesInstall = dialog.yesno(NAME, 'Voy a configurar GARGANTUA MultiVerse', nolabel='Cancelar', yeslabel='Continuar')
	if yesInstall:
	    save_check()
	    save_move1()
	    yesFresh = dialog.yesno('Borrar TODO', 'Recomendado borrar todo antes de configurar GARGANTUA MultiVerse.', nolabel='No', yeslabel='Borrar')
	    if yesFresh:
	    	freshStart()
...
def freshStart():
	...
	if yesFresh:
		...
		dp.create(addon_name, 'Borrar carpetas...')
		xbmc.sleep(1000)
		dp.update(30, 'Borrar carpetas...')
		xbmc.sleep(1000)
		...
		if not os.path.exists(packages):
			os.mkdir(packages)
		dp.update(100, 'Borrar Carpetas...Hecho')
		xbmc.sleep(2000)
		if mode == 4:
			save_move2()
...
def buildInstall(NAME, NAME2, VERSION, URL):
	...
	dp.update(100, 'Extrayendo...Hecho!')
	xbmc.sleep(2000)
	zf.close()
	os.unlink(zippath)
	save_move2()
