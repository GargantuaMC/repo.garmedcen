...
def get_movie_window(window_type):
	...
	class DialogVideoInfo(DialogBaseInfo, window_type):
		...
		def search_in_meta_by_title(self):
			url = 'plugin://plugin.video.acmeplayer/movies/tmdb/search_term/%s/1/' % self.info.get('title', '')
			self.close()
		...
		def play_movie(self):
			if self.dbid and int(self.dbid) > 0:
				...
				url = 'plugin://plugin.video.acmeplayer/movies/play/tmdb/%s' % self.info.get('id', '')
		...
		def play_movie_choose_player(self):
			if self.dbid and int(self.dbid) > 0:
				...
				url = 'plugin://plugin.video.acmeplayer/movies/play_choose_player/tmdb/%s/False' % self.info.get('id', '')
				xbmc.executebuiltin('RunPlugin(%s)' % url)

		@ch.click(445)
		def show_manage_dialog(self):
			manage_list = []
			manage_list.append(["ACME Info's settings", 'Addon.OpenSettings("script.extendedinfo")'])
			manage_list.append(["ACME Player's settings", 'Addon.OpenSettings("plugin.video.acmeplayer")'])
		...
		def add_movie_to_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.acmeplayer)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.acmeplayer/setup/total)')
			if xbmcgui.Dialog().yesno('ACME Info', 'Add [B]%s[/B] to library?' % self.info['title']):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.acmeplayer/movies/add_to_library/tmdb/%s)' % self.info.get('id', ''))
		...
		def remove_movie_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.acmeplayer)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.acmeplayer/setup/total)')
			if xbmcgui.Dialog().yesno('ACME Info', 'Remove [B]%s[/B] from library?' % self.info['title']):
