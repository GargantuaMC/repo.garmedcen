...
class MyProgressDialog():
    def __init__(self, process):
        ...
        self.dp.create("GARGANTÚA-MC", process, '', 'Espere...')
...
def get_packages():
    ...
    url = 'https://archive.org/download/GARMC2204RLS/GARMC2204RLS.zip'
    response = urllib.urlopen(url)
    try:
        data = read(response, MyProgressDialog("Descargando configuración GARGANTÚA-MC..."))
    except Canceled:
    ...
    dialog.ok(addon_name, "%s Por favor, cierra GARGANTÚA-MC para completar el proceso" % message)
    os._exit(0)
